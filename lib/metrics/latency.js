...
export function computeLatency(data) {
  ...
  const perResponseBreakdown = responseLogs.map((log, i) => {
    ...
    return {
      ...
      // acoustic_latency: "silence end → bot audio in", anchored at the
      // last detected-partial-speech and the first non-silence PCM frame
      // — closer to the user's perceived gap than audio_latency, which
      // is held open by the DG VAD-hang window. May be null on older calls.
      acousticLatency: log.acoustic_latency ?? null,
  ...
  // Parallel acoustic_latency stats — only computed when at least one
  // response carries the field. Surfaced alongside, not in place of,
  // the audio_latency-based headline so old/new calls remain comparable.
  const acousticItems = assistantResponses.filter(r => r.acousticLatency != null);
  let acousticStats = null;
  if (acousticItems.length > 0) {
    const vals = acousticItems.map(r => r.acousticLatency);
